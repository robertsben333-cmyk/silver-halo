...
def main():
    ...
    # --- Variable Exit Analysis (Time vs Confidence) ---
    if 'ModelScore' in df.columns:
        print("\n--- Running Variable Exit Analysis (By Confidence Tier) ---")
        
        # Create Tiers
        # Tier 1: Moderate (Score 2% - 4%)
        # Tier 2: High (Score > 4%)
        # Tier 3: Very High? (Just use >4 for now, sample size limited)
        
        # Let's try 3 bins if we have enough data, otherwise 2
        # Baseline: Score < 0 (All)
        # Moderate: 0 < Pred < 3.5
        # High: Pred > 3.5
        
        tiers = {
            "Moderate Confidence (0% < Pred < 3.5%)": df[(df['ModelScore'] > 0) & (df['ModelScore'] <= 3.5)].copy(),
            "High Confidence (Pred > 3.5%)": df[df['ModelScore'] > 3.5].copy()
        }
        
        combined_data = []
        
        for tier_name, tier_trades in tiers.items():
            if tier_trades.empty: continue
            
            print(f"Processing {tier_name} ({len(tier_trades)} trades)...")
            
            tier_results = []
            for idx, row in tier_trades.iterrows():
                # Re-use cached or fetch
                bars = get_minute_bars(row['ticker'], row['date'])
                if bars is None: continue
                
                pm = simulate_trade_path(bars)
                if pm:
                    pm['Tier'] = tier_name
                    tier_results.append(pm)
                    
            if tier_results:
                tier_df = pd.DataFrame(tier_results)
                melted = tier_df.melt(id_vars=['Tier'], value_vars=[30, 60, 90, 120, 180, 240, 360], var_name='Minutes', value_name='Return')
                melted['ReturnPct'] = melted['Return'] * 100
                melted['Hours'] = melted['Minutes'] / 60.0
                combined_data.append(melted)

        if combined_data:
            all_tiers = pd.concat(combined_data, ignore_index=True)
            
            # Save CSV
            all_tiers.to_csv(os.path.join(OUTPUT_DIR, "variable_exit_stats.csv"), index=False)
            
            # Plot Comparison
            plt.figure(figsize=(10, 6))
            sns.lineplot(data=all_tiers, x='Hours', y='ReturnPct', hue='Tier', style='Tier', markers=True, dashes=False, errorbar=None)
            
            plt.title("Optimal Exit Time by Model Confidence")
            plt.xlabel("Hours Since Market Open")
            plt.ylabel("Avg Return (%)")
            plt.axhline(0, color='black', linewidth=0.8)
            plt.grid(True)
            
            plot_path = os.path.join(OUTPUT_DIR, "variable_exit_comparison.png")
            plt.savefig(plot_path)
